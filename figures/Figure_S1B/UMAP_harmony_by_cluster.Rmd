...
output_figure_prefix <- "./figures/Figure_S1B/UMAP_Harmony_by_Cluster"
```
...
p <- DimPlot(data.seurat, reduction = "umap_harmony", label = T, pt.size = 0.5, group.by = "seurat_clusters", repel = F, cols = clusterPalette) + NoLegend()
p
```
...
p <- DimPlot(data.seurat, reduction = "umap_harmony", label = F, pt.size = 0.5, group.by = "annotation_clusters", cols = annotatedClusterPalette) + theme(legend.position = "bottom")  + guides(color = guide_legend(ncol = 9)) 
p
```

```{r}
ggsave(p, filename = paste0(output_figure_prefix, "_legend.pdf"), width = 30, height = 30, bg = "white")
```
