...
```{r}
#all.markers.2  <- FindMarkers(data.seurat, assay = "SCENIC", slot = "data", verbose = T, test.use = "LR", random.seed = 42, logfc.threshold = 0, min.pct = -Inf, latent.vars = "orig.ident", ident.1 = "2") # For unknown reasons Seuratv4 fails. So I use Seuratv5.
#all.markers.2$gene <- rownames(all.markers.2)
...
# Choose coloring
colors.volcano <- c("Non-significant" = "grey", 
                    "Upregulated" = "darkblue", 
                    "Upregulated (low effect size)" = "lightblue",
                    "Downregulated" = "darkred", 
                    "Downregulated (low effect size)" = "lightcoral")

# Genes to annotate
genes_to_label <- c("sage", "Hr39", "Atf-2", "gt", "sima", "crc", "CG10348", "CG6808", 
                    "esg", "bowl", "NC2alpha", "su(Hw)", "Mtk", "ovo", "Myc", "odd", 
                    "Mabi", "CG4374", "term", "CG1602")
