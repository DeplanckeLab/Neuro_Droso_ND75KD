...
output_figure_prefix <- "./figures/Figure_XXXX/"
```
...
# Plot regulon activity between 2 conditions

srl = spargel. Also known as dPGC-1, PGC-1α, PGC-1, dPGC1, PGC1-α. FBgn0037248
PGC-1α is not in the TF list I had.

# Plot regulon activity between 2 conditions

```{r, fig.width=17,fig.height=5}
# ewg = 	erect wing Also known as EC3, EG:BACR37P7.7 , l(1)1Ag, l(1)EC3. FBgn0005427. Main ortholog for NRF-1.
regulon <- "ewg"
alias <- "NRF-1"

# Prepare
boxplot_data <- data.frame(regulon_activity = data.seurat$SCENIC_ewg_activating_regulon, annotation = data.seurat$annotation_clusters)

# Calculate summary regulon_activity per annotation (cluster)
summary_activity <- tapply(boxplot_data$regulon_activity, boxplot_data$annotation, mean)

# Sort annotations by mean regulon_activity
sorted_annotations <- names(sort(summary_activity, decreasing = TRUE))

# Reorder the annotation factor
boxplot_data$annotation <- factor(boxplot_data$annotation, levels = sorted_annotations)

# Plot
p <- ggplot(boxplot_data, aes(x = annotation, y = regulon_activity, fill = annotation)) +
  geom_boxplot() +
  labs(x = NULL, y = paste0(regulon, " regulon activity\n(",alias,")")) +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_fill_manual(values = annotationClustersPalette) +
  NoLegend()

# Save
ggsave(p, filename = paste0(output_figure_prefix, "ordered_boxplot_",regulon,"_regulon_activity_CTRL.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "ordered_boxplot_",regulon,"_regulon_activity_CTRL.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
...
data.scatter <- data.frame(ewg_regulon_activity = data.seurat$SCENIC_ewg_activating_regulon, AUC_OXPHOS_68_Porcelli = data.seurat$AUC_OXPHOS_68_Porcelli)

# Create the scatter plot
p <- ggplot(data.scatter, aes(x = ewg_regulon_activity, y = AUC_OXPHOS_68_Porcelli)) +
  geom_point() +
  theme_minimal()

# Calculate correlation, p-value, and R²
cor_test <- cor.test(data.seurat$SCENIC_ewg_activating_regulon, data.seurat$AUC_OXPHOS_68_Porcelli, method = "pearson")
cor_value <- cor_test$estimate
p_value <- cor_test$p.value
r_squared <- cor_value^2

# Create the annotation text
annotation_text <- paste("Correlation: ", round(cor_value, 3),
                        "\nP-value: ", format(p_value, scientific = TRUE),
                        "\nR²: ", round(r_squared, 3), sep = "")

# Add the annotation to the plot
p <- p + annotate("text", x = Inf, y = Inf, label = annotation_text, hjust = 1.1, vjust = 1.1, size = 4)

# Save
ggsave(p, filename = paste0(output_figure_prefix, "scatterplot_",regulon,"_regulon_activity_correlation_CTRL.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "scatterplot_",regulon,"_regulon_activity_correlation_CTRL.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
...
```{r, fig.width=17,fig.height=5}
# Ets97D = Ets at 97D Also known as D-elg, Delg, elg. FBgn0004510.
regulon <- "Ets97D"
alias <- "GABPα/NRF-2"

# Prepare
boxplot_data <- data.frame(regulon_activity = data.seurat$SCENIC_Ets97D_activating_regulon, annotation = data.seurat$annotation_clusters)
...
data.scatter <- data.frame(Ets97D_regulon_activity = data.seurat$SCENIC_Ets97D_activating_regulon, AUC_OXPHOS_68_Porcelli = data.seurat$AUC_OXPHOS_68_Porcelli)

# Create the scatter plot
p <- ggplot(data.scatter, aes(x = Ets97D_regulon_activity, y = AUC_OXPHOS_68_Porcelli)) +
  geom_point() +
  theme_minimal()

# Calculate correlation, p-value, and R²
cor_test <- cor.test(data.seurat$SCENIC_Ets97D_activating_regulon, data.seurat$AUC_OXPHOS_68_Porcelli, method = "pearson")
cor_value <- cor_test$estimate
p_value <- cor_test$p.value
r_squared <- cor_value^2
...
```{r, fig.width=17,fig.height=5}
# cnc = 	cap-n-collar Also known as Nrf2, CncC, dNrf2, cap and collar, Cap 'n' collar. FBgn0262975 Main ortholog for NRF-2.
regulon <- "cnc"
alias <- "NRF-2 ?"

# Prepare
boxplot_data <- data.frame(regulon_activity = data.seurat$SCENIC_cnc_activating_regulon, annotation = data.seurat$annotation_clusters)
...
data.scatter <- data.frame(cnc_regulon_activity = data.seurat$SCENIC_cnc_activating_regulon, AUC_OXPHOS_68_Porcelli = data.seurat$AUC_OXPHOS_68_Porcelli)

# Create the scatter plot
p <- ggplot(data.scatter, aes(x = cnc_regulon_activity, y = AUC_OXPHOS_68_Porcelli)) +
  geom_point() +
  theme_minimal()

# Calculate correlation, p-value, and R²
cor_test <- cor.test(data.seurat$SCENIC_cnc_activating_regulon, data.seurat$AUC_OXPHOS_68_Porcelli, method = "pearson")
cor_value <- cor_test$estimate
p_value <- cor_test$p.value
r_squared <- cor_value^2
...
Kdm5 = Lysine demethylase 5 Also known as lid, little imaginal discs, l(2)10424, dKDM5/LID. FBgn0031759
gene <- "lid"
alias <- "Kdm5"

It's not in our list of TFs
