...
median_values <- tapply(boxplot_data$AUC_OXPHOS_68_Porcelli, boxplot_data$annotation_clusters, median)

# Sorting the metadata based on median values
boxplot_data$annotation_clusters <- factor(boxplot_data$annotation_clusters, levels = names(sort(median_values, decreasing = F)))

# Plot
p <- ggplot(boxplot_data, aes(x = annotation_clusters, y = AUC_OXPHOS_68_Porcelli, fill = annotation_clusters)) +
  geom_boxplot() +
...
```{r}
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
```

## Boxplot (Control only)

```{r, fig.width=15,fig.height=5}
# Extracting the relevant metadata
boxplot_data <- as.data.frame(data.seurat[,data.seurat$orig.ident == "Pan_neuro_control"]@meta.data)

# Filter out <5 cells per annotation
boxplot_data <- boxplot_data[boxplot_data$seurat_clusters %in% names(which(table(boxplot_data$seurat_clusters) >= 5)),]
...
```{r}
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot_control-only.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot_control-only.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
```

## Boxplot (ND-75 KD only)

```{r, fig.width=15,fig.height=5}
# Extracting the relevant metadata
boxplot_data <- as.data.frame(data.seurat[,data.seurat$orig.ident == "Pan_neuro_ND75KD"]@meta.data)
...
```{r}
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot_nd75kd-only.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "Oxphos_68_ordered_boxplot_nd75kd-only.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
```