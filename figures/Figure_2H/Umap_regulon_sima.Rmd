...
output_figure_prefix <- "./figures/Figure_2H/UMAP_Regulon_sima"
```
...
p <- FeaturePlot(data.seurat, reduction = "SCENICbinarizedumap", pt.size = 0.5, features = "SCENIC_sima_activating_regulon", order = T) + 
  scale_colour_gradientn(colors = c("#f2f2f2", "#e6e6e6", "#cd1c18"), values = scales::rescale(c(0, 0.1, max(data.seurat@meta.data$SCENIC_crc_activating_regulon, na.rm = TRUE)))) + NoLegend()
p
```
