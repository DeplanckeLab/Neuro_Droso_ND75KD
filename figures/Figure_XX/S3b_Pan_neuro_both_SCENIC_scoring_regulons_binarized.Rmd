...
## V. Displaying regulons / gene expressions
...
## VII.4 Statistical significance? (t-test)
...
```{r}
data.sig <- data.frame(annotation_clusters = unique(data.seurat$annotation_clusters), p.value = 1, log.fold.change = 0)
rownames(data.sig) = as.character(data.sig$annotation_clusters)
for(clust in rownames(data.sig)){
  ctrl <- data.seurat$AUC_OXPHOS_117_Flybase[data.seurat$orig.ident == "Pan_neuro_control" & data.seurat$annotation_clusters == clust]
  nd75kd <- data.seurat$AUC_OXPHOS_117_Flybase[data.seurat$orig.ident == "Pan_neuro_ND75KD" & data.seurat$annotation_clusters == clust]
  if(length(ctrl) < 5 | length(nd75kd) < 5){
    data.sig[clust, "p.value"] <- NA
    data.sig[clust, "log.fold.change"] <- NA
  } else {
    data.sig[clust, "p.value"] <- t.test(ctrl, nd75kd)$p.value
...
**Of note:** Our cluster 2 & 23 of interest are not there because there are not enough cells in CTRL to compute p-value
- Cluster 23: 2 ctrl cells vs 331 ND-75KD cells
- Cluster 2: 4 ctrl cells vs 813 ND-75KD cells

```{r}
ggplotly(ggplot(data.sig, aes(x = log.fold.change, y = -log10(p.value), text = annotation_clusters)) + geom_point() + ggtitle("Volcano plot - Ctrl vs ND-75KD - 117 OXPHOS genes"))
```

```{r, fig.width = 12}
data.sig$annotation_clusters <- factor(data.sig$annotation_clusters, levels = data.sig$annotation_clusters)
ggplot(data.sig, aes(y = -log10(p.value), x = annotation_clusters)) + geom_bar(stat="identity") + ggtitle("t-test p-values - Ctrl vs ND-75KD - 117 OXPHOS genes") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
# Calculating median values of "AUC_OXPHOS_117_Flybase" for each group
median_values <- tapply(metadata$AUC_OXPHOS_117_Flybase, metadata$annotation, median)

# Sorting the metadata based on median values
metadata$annotation <- factor(metadata$annotation, levels = names(sort(median_values, decreasing = F)))

# Plotting a violin plot using ggplot2
ggplot(metadata, aes(x = annotation, y = AUC_OXPHOS_117_Flybase)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Seurat Clusters", y = "AUC_OXPHOS_117_Flybase")
...
# Calculating median values of "AUC_OXPHOS_117_Flybase" for each group
median_values <- tapply(metadata$AUC_OXPHOS_117_Flybase, metadata$annotation_clusters, median)

# Sorting the metadata based on median values
metadata$annotation_clusters <- factor(metadata$annotation_clusters, levels = names(sort(median_values, decreasing = F)))

# Plotting a violin plot using ggplot2
ggplot(metadata, aes(x = annotation_clusters, y = AUC_OXPHOS_117_Flybase)) +
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Seurat Clusters / Emma's Annotation", y = "AUC_OXPHOS_117_Flybase")
...
for(exp in c("Pan_neuro_control", "Pan_neuro_ND75KD")){
  ...
  # Plotting a violin plot using ggplot2
  p[[exp]] <- ggplot(metadata, aes(x = annotation_clusters, y = AUC_OXPHOS_117_Flybase)) +
    geom_boxplot() +
    theme_minimal() +
    theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    labs(x = "Seurat Clusters / Emma's Annotation", y = "AUC_OXPHOS_117_Flybase") + ggtitle(exp)
...
```{r, fig.height=15, fig.width=15}
DimPlot(data.seurat, reduction = "tsne_harmony", label = TRUE, pt.size = 0.5, group.by = "annotation") + NoLegend()
```

```{r, fig.height=15, fig.width=15}
DimPlot(data.seurat, reduction = "tsne_harmony", label = TRUE, pt.size = 0.5, group.by = "annotation_clusters") + NoLegend()
```
...
rownames(data.sig) = as.character(data.sig$annotation_clusters)
for(clust in rownames(data.sig)){
  ctrl <- data.seurat$SCENIC_crc_activating_regulon[data.seurat$orig.ident == "Pan_neuro_control" & data.seurat$annotation_clusters == clust]
  nd75kd <- data.seurat$SCENIC_crc_activating_regulon[data.seurat$orig.ident == "Pan_neuro_ND75KD" & data.seurat$annotation_clusters == clust]
  if(length(ctrl) < 5 | length(nd75kd) < 5){
    data.sig[clust, "p.value"] <- NA
    data.sig[clust, "log.fold.change"] <- NA
  } else {
...
```{r}
ggplotly(ggplot(data.sig, aes(x = log.fold.change, y = -log10(p.value), text = annotation_clusters)) + geom_point() + ggtitle("Volcano plot - Ctrl vs ND-75KD - Regulon crc (Atf4)"))
```

```{r, fig.width = 12}
data.sig$annotation_clusters <- factor(data.sig$annotation_clusters, levels = data.sig$annotation_clusters)
ggplot(data.sig, aes(y = -log10(p.value), x = annotation_clusters)) + geom_bar(stat="identity") + ggtitle("t-test p-values - Ctrl vs ND-75KD - Regulon crc (Atf4)") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
## V. Imp vs OXPHOS
