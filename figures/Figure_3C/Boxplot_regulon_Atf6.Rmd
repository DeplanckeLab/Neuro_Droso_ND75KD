...
output_figure_prefix <- "./figures/Figure_3C/Boxplot_Regulon_Atf6"
```
...
data.boxplot <- FetchData(data.seurat, vars = c("seurat_clusters", "SCENIC_Atf6_activating_regulon"))
...
```{r}
stats <- wilcox.test(data.boxplot$SCENIC_Atf6_activating_regulon[data.boxplot$seurat_clusters == 2], data.boxplot$SCENIC_Atf6_activating_regulon[data.boxplot$seurat_clusters == 23])
stats
```
...
p <- ggplot(data.boxplot, aes(x = seurat_clusters, y = SCENIC_Atf6_activating_regulon, fill = seurat_clusters)) +
  geom_boxplot() +
  scale_fill_manual(values = clusterPalette) +
  labs(y = "Atf6 regulon activity") +
  theme_minimal() +
  NoLegend() +
  geom_signif(size = 0.5, textsize = 4,
    ...
    y_position = max(data.boxplot$SCENIC_Atf6_activating_regulon) + 0.05 * max(data.boxplot$SCENIC_Atf6_activating_regulon), # Position above the boxes
    tip_length = 0.02
  )
p
```
