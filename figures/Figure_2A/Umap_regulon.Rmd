...
output_figure_prefix <- "./figures/Figure_2A/UMAP_Regulon_By_Cluster"
```
...
p <- DimPlot(data.seurat, reduction = "SCENICumap", label = T, pt.size = 0.5, group.by = "seurat_clusters", repel = F, cols = clusterPalette) + NoLegend()
p
```

```{r, fig.height = 12, fig.width = 12}
p <- DimPlot(data.seurat, reduction = "SCENICbinarizedumap", label = T, pt.size = 0.5, group.by = "seurat_clusters", repel = F, cols = clusterPalette) + NoLegend()
p
```
