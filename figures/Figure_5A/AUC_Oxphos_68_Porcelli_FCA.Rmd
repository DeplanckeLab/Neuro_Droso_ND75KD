...
```{r}
input_seurat_object <- "./data/Fly_Cell_Atlas__Head_Stringent.rds"
input_Oxphos_68_AUCell_values_path <- "./data/FCA_68_Oxphos_AUCell_auc.tsv"
...
## II.1 Reading OXPHOS AUC values from AUCell (using the 68 OXPHOS genes from Porcelli et al. https://link.springer.com/article/10.1186/1471-2148-7-215)
...
hist(data.seurat$AUC_OXPHOS_68_Porcelli, breaks = 100)
```
...
boxplot_data <- boxplot_data[boxplot_data$annotation %in% names(which(table(boxplot_data$annotation) >= 5)),]

# Filter out artefact & unnanotated
boxplot_data <- boxplot_data[!boxplot_data$annotation %in% c("unannotated", "artefact"),]

# Calculating median values of "AUC_OXPHOS_68_Porcelli" for each group
median_values <- tapply(boxplot_data$AUC_OXPHOS_68_Porcelli, boxplot_data$annotation, median)

# Sorting the metadata based on median values
boxplot_data$annotation <- factor(boxplot_data$annotation, levels = names(sort(median_values, decreasing = F)))

# Plot
p <- ggplot(boxplot_data, aes(x = annotation, y = AUC_OXPHOS_68_Porcelli, fill = annotation)) +
  geom_boxplot() +
...
```{r}
ggsave(p, filename = paste0(output_figure_prefix, "FCA_HEAD_Oxphos_68_ordered_boxplot.pdf"), width = 15, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, "FCA_HEAD_Oxphos_68_ordered_boxplot.png"), width = 15, height = 5, dpi = 1000, bg = 'white')
```
