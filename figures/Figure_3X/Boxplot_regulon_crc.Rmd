...
suppressPackageStartupMessages(library(ggsignif)) # For grid plotting
...
cat(bold("ggsignif"), "version", as.character(packageVersion("ggsignif")), "\n")
...
output_figure_prefix <- "./figures/Figure_2Fb/Boxplot_Regulon_crc"
```
...
data.boxplot <- FetchData(data.seurat, vars = c("seurat_clusters", "SCENIC_crc_activating_regulon"))

# Filter for clusters 2 and 23
data.boxplot <- subset(data.boxplot, seurat_clusters %in% c(2, 23))

# Convert clusters to a factor for better plotting
data.boxplot$seurat_clusters <- factor(data.boxplot$seurat_clusters, levels = c(2, 23))
```

# Statistical analysis

```{r}
stats <- wilcox.test(data.boxplot$SCENIC_crc_activating_regulon[data.boxplot$seurat_clusters == 2], data.boxplot$SCENIC_crc_activating_regulon[data.boxplot$seurat_clusters == 23])
stats
```
...
significance_label <- ifelse( # This is ugly
  stats$p.value > 0.05, "ns",
  ifelse(stats$p.value <= 0.0001, "****",
         ifelse(stats$p.value <= 0.001, "***",
                ifelse(stats$p.value <= 0.01, "**", "*")))
)

p <- ggplot(data.boxplot, aes(x = seurat_clusters, y = SCENIC_crc_activating_regulon, fill = seurat_clusters)) +
  geom_boxplot() +
  scale_fill_manual(values = clusterPalette) +
  labs(y = "SCENIC_crc_activating_regulon") +
  theme_minimal() +
  NoLegend() +
  geom_signif(size = 0.5, textsize = 4,
    comparisons = list(c("2", "23")), # Specify the clusters being compared
    annotations = significance_label, # Add star if significant
    y_position = max(data.boxplot$SCENIC_crc_activating_regulon) + 0.05 * max(data.boxplot$SCENIC_crc_activating_regulon), # Position above the boxes
    tip_length = 0.02
  )
p
```
...
```{r}
ggsave(p, filename = paste0(output_figure_prefix, ".pdf"), width = 2, height = 4, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, ".png"), width = 2, height = 4, dpi = 1000, bg = 'white')
```
