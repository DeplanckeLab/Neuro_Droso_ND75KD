...
output_figure_prefix <- "./figures/Figure_1D/Barplot_Ordered_Proportion_Cells_annotations"
```
...
for(cluster in names(table(data.seurat@meta.data$annotation_clusters))){
  which.cells <- (data.seurat@meta.data$annotation_clusters == cluster)
...
data.barplot$nbcells <- as.numeric(data.barplot$nbcells)
data.barplot$cluster <- as.character(data.barplot$cluster)
data.barplot$library <- as.character(data.barplot$library)

# Calculate proportions for ND-75 KD vs Control
unique_clusters <- unique(data.barplot$cluster)
proportions <- data.frame(cluster = unique_clusters, proportion_nd75kd = 0)
for (cl in unique_clusters) {
  subset_cluster <- data.barplot[data.barplot$cluster == cl, ]
  total_cells <- sum(subset_cluster$nbcells)
  nd75kd_cells <- sum(subset_cluster$nbcells[subset_cluster$library == "Pan_neuro_ND75KD"])
  proportions$proportion_nd75kd[proportions$cluster == cl] <- nd75kd_cells / total_cells
}

# Sort clusters by proportion of ND75KD cells
sorted_clusters <- proportions$cluster[order(-proportions$proportion_nd75kd)]

# Reorder data.barplot based on sorted clusters
data.barplot$cluster <- factor(data.barplot$cluster, levels = sorted_clusters)

# plot
p <- ggplot(data.barplot, aes(fill=library, y=nbcells, x=cluster)) + 
