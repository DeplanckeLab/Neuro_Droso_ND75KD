...
description: "Figure - DE genes vs cell count"
...
```{r}
input_de_file <- "./figures/Figure_1DE/List_DE_ND75KD_Control_all_annotations.tsv"
input_seurat_object <- "./data/Pan_neuro_integrated_FINAL.rds"
output_figure_prefix <- "./figures/Figure_S1C/Scatterplot_DEG_vs_cellcount"
```
...
```{r}
# Load pre-computed significant DE genes 
all.de.genes <- fread(input_de_file, sep = "\t", quote = F, header = T, stringsAsFactors = F, data.table = F)
all.de.genes
```

## Cell count per annotation

```{r}
all_annotations <- sort(unique(data.seurat$annotation))
all_annotations <- all_annotations[all_annotations != "unannotated"]
results <- data.frame(row.names = all_annotations, annotation = all_annotations, cell_count = NA, Nb.DEG = NA)
for(annot in all_annotations){
  results[annot, "Nb.DEG"] <- sum(all.de.genes$annotation == annot)
  results[annot, "cell_count"] <- sum(data.seurat$annotation == annot)
}
results
```
...
p <- ggplot(data = results, aes(x = Nb.DEG, y = cell_count)) + 
  geom_point() +
  theme_minimal() +
  scale_x_continuous(breaks = 0:12, limits = c(0, 12)) +
  theme(panel.grid.minor.x = element_blank()) # Remove minor horizontal grid lines
p
```

```{r}
ggsave(p, filename = paste0(output_figure_prefix, ".pdf"), width = 5, height = 5, bg = "white")
ggsave(p, filename = paste0(output_figure_prefix, ".png"), width = 5, height = 5, dpi = 1000, bg = 'white')
```

```{r}
cor.test(results$cell_count, results$Nb.DEG)
```

```{r}
summary(lm(results$cell_count ~ results$Nb.DEG))
```