...
suppressPackageStartupMessages(library(reticulate)) # For writing DE gene file
...
#adata <- anndata$AnnData(X = Matrix::t(X), raw = list(X = Matrix::t(raw.X)), obs = obs, var = var, obsm = obsm)
adata <- anndata$AnnData(X = Matrix::t(X), raw = Matrix::t(raw.X), obs = obs, var = var, obsm = obsm)

# Convert to pandas using reticulate if needed
pandas <- reticulate::import("pandas")
adata$raw$var <- pandas$DataFrame(adata$raw$var)
adata$raw$var <- adata$raw$var$reset_index(drop = TRUE)
