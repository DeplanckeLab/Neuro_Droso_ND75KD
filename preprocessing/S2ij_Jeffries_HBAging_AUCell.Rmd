---
title: "Emma Pan Neuro (Control + ND75KD) - Preprocessing and plotting Jeffries et al. The Human Brain Aging dataset."
...
```{r}
suppressPackageStartupMessages(library(Seurat)) # Seurat v5 for compatibility with Jeffries' object
suppressPackageStartupMessages(library(SeuratObject)) # For compatibility purpose
...
*Loom / H5ad file:* NA. Made from .rds file.

```{r}
## Parameters
input_rds <- "./data/Jeffries_2025_HumanBrainAging/pfc.clean.rds" # 367'317 cells vs 29'729 genes
input_Oxphos_136_AUCell_values_path <- "./data/Jeffries_2025_HumanBrainAging_116_Oxphos_AUCell_auc.tsv"
input_UPR_93_AUCell_values_path <- "./data/Jeffries_2025_HumanBrainAging_90_UPR_AUCell_auc.tsv"
input_SCENIC_regulons_path <- "./data/Jeffries_2025_HumanBrainAging/pySCENIC/Martirosyan_regulons_aucell.tsv"
input_SCENIC_regulons_binarized_path <- "./data/Jeffries_2025_HumanBrainAging/pySCENIC/Martirosyan_regulons_aucell_binarized.tsv"

output_seurat_final <- "./data/Jeffries_2025_HumanBrainAging/Jeffries_HBAging"
output_de_folder <- "./data/Jeffries_2025_HumanBrainAging/DE/"
```
...
### I.2 Reading OXPHOS AUC values from AUCell (using the 136 OXPHOS genes from KEGG Oxphos pathway https://www.genome.jp/entry/hsa00190)

```{r}
data.auc.oxphos <- fread(input_Oxphos_136_AUCell_values_path, data.table = F, check.names = F)
rownames(data.auc.oxphos) <- data.auc.oxphos$Cell
data.auc.oxphos
```

### I.3  Reading UPR AUC values from AUCell (using the 93 UPR genes from REACTOME - UNFOLDED PROTEIN RESPONSE (UPR))
...
### I.4 Add to Seurat Object

```{r}
# Add metadata
data.seurat$AUC_OXPHOS_136_KEGG <- data.auc.oxphos[colnames(data.seurat), "AUC"]
