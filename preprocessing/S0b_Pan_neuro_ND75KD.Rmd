...
```{r}
# Also setting up the default repository for the analysis (TODO: CHANGE THIS)
output_10x <- "./data/Pan_neuro_ND75KD.h5"
project_name <- "Pan_neuro_ND75KD"
```
...
The p-value distribution show that ~80 PCs are good (similar to Pan_neuro_control), so I keep 80 PCs for the rest of the pipeline.
I've plotted the tail of the JackStraw plot (70 to 90 PCs) to see that indeed the sweet spot is around there.

```{r}
JackStrawPlot(data.seurat, dims = 70:90)
```

# Clustering
```{r}
data.seurat <- FindNeighbors(data.seurat, dims = 1:80)
data.seurat <- FindClusters(data.seurat, resolution = 4)
```

# UMAP (visualization)
```{r}
data.seurat <- RunUMAP(data.seurat, dims = 1:80)
DimPlot(data.seurat, reduction = "umap")
```

# t-SNE (visualization)
```{r}
data.seurat <- RunTSNE(data.seurat, dims = 1:80)
